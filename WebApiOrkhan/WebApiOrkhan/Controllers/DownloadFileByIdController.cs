...
namespace WebApiOrkhan.Controllers
{
    ...
    public class DownloadFileByIdController : Controller
    {
        /*Скачивание конкретного материала
         1)Можно реализовать поиск по конкретному id file в таблице files
         2)Нужно создать поле в таблице files с порядковым номером версии конкретного материала
         --можно попробовать реализовать метод который позволит записывать номер версии при загрузке*/
